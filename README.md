...
[실행 방법](#설치-없이-실행하기) · [사용 순서](#사용-순서) · [단축키](#단축키) · [변경 이력](CHANGELOG.md) · [선택 기준과 비교 검토](docs/POSITIONING.md)
...
**시작 조건:** Windows x64, 한국어·영어 UI. **Language / 언어** 메뉴에서 바로 전환하며 선택을 기억합니다. 첫 실행은 시스템 언어가 한국어이면 한국어, 나머지는 영어입니다. 정밀 화질 분석이나 프레임 단위 계측용 도구가 필요한 경우에는 [동작 범위와 제한](#동작-범위와-제한)을 먼저 확인하세요.

## 설치 없이 실행하기

포터블 패키지를 받았다면 **ZIP 전체 압축 해제 → SyncView/SyncView.exe 실행**만 하면 됩니다. Python·FFmpeg를 따로 설치할 필요가 없습니다. `_internal` 폴더를 EXE와 함께 보관하세요. [빠른 시작](docs/QUICKSTART.md)에 사용 순서가 있습니다.

현재 GitHub에는 소스를 제공하며, **일반 사용자용 ZIP은 아직 릴리스에 게시하지 않았습니다.** 포터블 빌드와 로컬 실행은 검증했습니다. 공개 다운로드는 [외부 라이선스 자료 준비](docs/LICENSING.md)를 완료한 뒤 제공합니다. 소스로 실행하려면 아래 개발 환경 설치를 따르세요.
...
| 언어 선택 | 한국어·English 즉시 전환, 선택 저장, 두 언어의 내장 도움말 |
...
포터블 폴더와 ZIP 후보를 만들려면 다음 명령을 사용합니다. 테스트·정적 검사·FFmpeg 준비·빌드·패키징을 순서대로 수행합니다.

~~~powershell
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/build_portable.ps1
~~~

결과는 **dist/SyncView-windows-x64-portable.zip**과 `.zip.sha256`입니다. 실행 폴더는 `dist/portable/SyncView/`입니다. ZIP에는 빠른 시작 안내, 라이선스 고지, 앱 소스 스냅샷, 파일별 해시를 담은 `build-info.json`도 들어갑니다. 외부 라이브러리 대응 소스는 별도 준비 대상이므로 이 ZIP 생성만으로 공개 배포 준비가 끝나는 것은 아닙니다.

언어를 이번 실행에서만 지정하려면 `python -m videos_multi_view --language en` 또는 `SyncView.exe --language ko`를 사용합니다. 메뉴에서 선택한 언어는 Windows 사용자 설정에 저장되며 JSON 프로젝트와 원본 이름표는 바꾸지 않습니다.
